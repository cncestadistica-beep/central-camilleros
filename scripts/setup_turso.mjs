...
if (!url || !authToken) {
  console.error('Error: Por favor coloca TURSO_DATABASE_URL y TURSO_AUTH_TOKEN en tu archivo .env');
  process.exit(1);
}
...
async function setup() {
  console.log('Conectando a Turso SQLite en:', url);
  ...
  for (const c of camilleros) {
    await client.execute({
      sql: 'INSERT OR IGNORE INTO camilleros_personal (name) VALUES (?)',
      args: [c],
    });
  }
  ...
  for (const s of demoRequests) {
    await client.execute({
      sql: `
        INSERT OR IGNORE INTO solicitudes_camilleros (
  ...
  const res = await client.execute('SELECT count(*) as count FROM solicitudes_camilleros');
  console.log('✓ Tablas creadas e inicializadas en Turso SQLite con éxito!');
  console.log('✓ Total registros en Turso:', res.rows[0].count);
}

setup().catch(err => {
  console.error('Error configurando Turso:', err);
});
