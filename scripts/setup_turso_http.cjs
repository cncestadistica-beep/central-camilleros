...
async function setup() {
  console.log('Creando tablas en Turso Cloud Database...');
  ...
  await executeTurso(stmts);

  const check = await executeTurso([
    'SELECT count(*) as count FROM solicitudes_camilleros'
  ]);
  const count = check.results[0].response.result.rows[0][0].value;
  console.log('✓ Tablas creadas y pobladas en tu base de Turso con éxito!');
  console.log('✓ Total solicitudes en Turso:', count);
}
